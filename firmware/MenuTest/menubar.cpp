...
void MenuBar::addItem(const char *text, float value, float *var) {
    menu_item_t item;
...
void MenuBar::addItem(const char *text, void (*func)()) {
    menu_item_t item;
    strncpy(item.display_text, text, 15);
    item.var = nullptr;
    item.value = 0.0f;
...
void MenuBar::executeItem(uint8_t item_index) {
  ...
  if (item_counts[category_selected] != 0) {
    ...
    strncpy(selected_text[category_selected], item.display_text, 16);
...
void MenuBar::drawMenu(ILI9341_t3n &tft) {
    ...
    for (uint8_t i = 0; i < num_categories; i++) {
        ...
        if (show_selections[i]) text_y = tab_y + 3;
        ...
        if (show_selections[i]) {
          ...
          text_x = tab_x + (tab_width - w) / 2;
          tft.setCursor(text_x, tab_y + category_height/2 + 2);
